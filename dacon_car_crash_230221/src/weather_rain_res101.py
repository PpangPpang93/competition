...
def train(model, optimizer, train_loader, val_loader, scheduler, device):
    ...
    for epoch in range(1, CFG['EPOCHS']+1):
        ...
        torch.save(model.state_dict(), '/data/home/ubuntu/workspace/dacon/ckp/weather_rain_res101_{0:02d}.ckpt'.format(epoch))
...
if __name__ == '__main__':
    ...
    if (args.mode == 'train'):
        ...
        df = pd.read_csv('/data/home/ubuntu/workspace/dacon/data/train_weather_rain.csv')
        
        total_frames, total_labels = [], []
        for i in tqdm(range(len(df))):
            frames, labels = get_img(df.loc[i,'video_path'], df.loc[i,'rain'])
        ...
        # wandb
        wandb.login()
        wandb.init(project='weather_rain_resnet101', config=CFG)
        # wandb.config
        ...
        test['rain'] = preds
        test.to_csv('/data/home/ubuntu/workspace/dacon/data/submission/test_weather_rain_230312.csv', index=False)