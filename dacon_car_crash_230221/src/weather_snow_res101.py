...
def train(model, optimizer, train_loader, val_loader, scheduler, device):
    ...
    for epoch in range(1, CFG['EPOCHS']+1):
        ...
        torch.save(model.state_dict(), '/data/home/ubuntu/workspace/dacon/ckp/weather_snow_res101_{0:02d}.ckpt'.format(epoch))
...
if __name__ == '__main__':
    ...
    if (args.mode == 'train'):
        ...
        df = pd.read_csv('/data/home/ubuntu/workspace/dacon/data/train_weather_snow.csv')
        
        total_frames, total_labels = [], []
        for i in tqdm(range(len(df))):
            frames, labels = get_img(df.loc[i,'video_path'], df.loc[i,'snow'])
        ...
        # wandb
        wandb.login()
        wandb.init(project='weather_snow_resnet101', config=CFG)
        # wandb.config
        ...
        test['snow'] = preds
        test.to_csv('/data/home/ubuntu/workspace/dacon/data/submission/test_weather_snow_230312.csv', index=False)